...
class CartController extends Controller
{
    ...
    function Add($id, Request $request)
    {
        ...
        $validator->validate();
        if ($validator->fails()) {
            return response()->json(['warning'=> 'الكتاب غير متاح']);
        }
        ...
        if (!$this->HandleCart()) {
            ...
            return response()->json(['success'=> 'تمت الاضافة للسلة بنجاح']);
        } else {
            $cart_id = Cart::where('user_id', Auth::guard('sanctum')->user()->id)->get('id');
            ...
            if ($this->CheckBookInCartItems($id,$item_data['cart_id'])) {
                ...
                $total = $this->total();
                $data['total'] = $total;
                Cart::where('user_id', Auth::guard('sanctum')->user()->id)->update($data);
                return response()->json(['success'=> 'تمت الاضافة للسلة بنجاح']);
            }
            Cart_items::create($item_data);
            $total = $this->total();
    ...
    function HandleCart()
    {
        $cart = Cart::where('user_id', Auth::guard('sanctum')->user()->id)->get('id');
        if (!empty($cart[0])) {
            $total = $this->total();
            $data['total'] = $total;
            Cart::where('user_id', Auth::guard('sanctum')->user()->id)->update($data);
    ...
    function GetCartPrice(){
        if($this->CheckCart()){
            ...
            $price = Cart::where('user_id', Auth::guard('sanctum')->user()->id)->get();
    ...
    function destroy(){
        Cart::where('user_id',Auth::guard('sanctum')->user()->id)->delete();
    }
}
